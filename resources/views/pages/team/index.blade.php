                        ...
                        <div class="card-body">
                            ...
                            <table class="table datatable hero-list">
                                ...
                                <tbody>
                                    ...
                                    @if (count($teamContents) > 0)
                                        @foreach ($teamContents as $lists)
                                            <tr>
                                                <td class="align-middle">{{ $loop->index+1 }}</td>
                                                <td class="align-middle">{{ $lists->name }}</td>
                                                <td class="align-middle">{{$lists->designation}}
                                                </td>
                                                <td class="align-middle hero-thumbnail">
                                                    <img src="{{ url('/storage/img/' . $lists->person_img) }}">
                                                </td>
                                                ...
                                                <td class="align-middle">
                                                    <div class="raw">
                                                        @if ($lists->status === 'active')
                                                            <a href="{{ route('pendingMember', $lists->id) }}"
                                                                title="Pending" class="btn btn-warning"><i
                                                                    class="bi bi-arrow-down-circle"></i></a>
                                                        @else
                                                            <a href="{{ route('activeMember', $lists->id) }}" title="Active"
                                                                class="btn btn-success"><i
                                                                    class="bi bi-check-circle"></i></a>
                                                        @endif

                                                        <a href="{{ route('editTeamList', $lists->id) }}"
                                                            class="btn btn-primary"><i class="fa-solid fa-pencil"></i></a>
                                                        <a href="{{ route('deleteTeamMember', $lists->id) }}"
